...
void TrayIcon::Update(int batteryLevel, bool charging, RazerDeviceType type) {
    ...
    Shell_NotifyIcon(NIM_ADD, &nid); // Ensure added
    Shell_NotifyIcon(NIM_MODIFY, &nid);
...
HICON TrayIcon::CreateBatteryIcon(int level, bool charging, RazerDeviceType type) {
    ...
    // Draw Battery Bar (Bottom) or Text? Text is requested.
    // "M" on top, "90" on bottom.
    ...
    ICONINFO ii = {0};
    ii.fIcon = TRUE;
    ii.hbmMask = hBitmap; // Use same bitmap for mask (simple transparency)
    ii.hbmColor = hBitmap;
    HICON hIcon = CreateIconIndirect(&ii);
