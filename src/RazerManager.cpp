...
void RazerManager::AddDevice(const std::wstring& path) {
    ...
    if (device->Connect()) {
        ...
        if (level == -1) {
             ...
             if (device->IsGeneric()) {
                 Logger::Instance().Log(L"Generic Interface connected but GetBatteryLevel failed (skipping): " + path);
                 return;
             } else {
                 Logger::Instance().Log(L"Vendor Interface connected but GetBatteryLevel failed (adding as Unknown): " + path);
                 // Fallthrough to add device
             }
        }
